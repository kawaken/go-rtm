package tasks

// This file is generated by methodGenerator.
// DO NOT MOTIFY THIS FILE.

import (
	"net/url"

	"github.com/kawaken/go-rtm/methods"
)

// SetRecurrence returns "rtm.tasks.setRecurrence" method instance.
func SetRecurrence(timeline string, listID string, taskseriesID string, taskID string, repeat string) *methods.Method {
	name := "rtm.tasks.setRecurrence"

	p := url.Values{}
	p.Add("method", name)
	p.Add("timeline", timeline)
	p.Add("list_id", listID)
	p.Add("taskseries_id", taskseriesID)
	p.Add("task_id", taskID)
	p.Add("repeat", repeat)
	
	return &methods.Method{Name: name, Params: p}
}
