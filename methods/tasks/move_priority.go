package tasks

// This file is generated by methodGenerator.
// DO NOT MOTIFY THIS FILE.

import (
	"net/url"

	"github.com/kawaken/go-rtm/methods"
)

// MovePriority returns "rtm.tasks.movePriority" method instance.
func MovePriority(timeline string, listID string, taskseriesID string, taskID string, direction string) *methods.Method {
	name := "rtm.tasks.movePriority"

	p := url.Values{}
	p.Add("method", name)
	p.Add("timeline", timeline)
	p.Add("list_id", listID)
	p.Add("taskseries_id", taskseriesID)
	p.Add("task_id", taskID)
	p.Add("direction", direction)
	
	return &methods.Method{Name: name, Params: p}
}
