package tasks

// This file is generated by methodGenerator.
// DO NOT MOTIFY THIS FILE.

import (
	"net/url"

	"github.com/kawaken/go-rtm/methods"
)

// SetPriority returns "rtm.tasks.setPriority" method instance.
func SetPriority(timeline string, listID string, taskseriesID string, taskID string, priority string) *methods.Method {
	name := "rtm.tasks.setPriority"

	p := url.Values{}
	p.Add("method", name)
	p.Add("timeline", timeline)
	p.Add("list_id", listID)
	p.Add("taskseries_id", taskseriesID)
	p.Add("task_id", taskID)
	p.Add("priority", priority)
	
	return &methods.Method{Name: name, Params: p}
}
